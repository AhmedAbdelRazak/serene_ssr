...
export const toggleCaseAiResponder = (caseId, userId, token, aiToRespond) => {
  return fetch(
    `${process.env.REACT_APP_API_URL}/support-cases/${caseId}/ai-responder/${userId}`,
    {
      method: "PUT",
      headers: {
        Accept: "application/json",
      ...
      body: JSON.stringify({ aiToRespond }),
    },
  )
    .then(async (res) => {
    ...
    .catch((err) => {
      console.error("Error toggling case AI responder:", err);
      throw err;
    });
};
...
export const getWebsiteSetupPublic = () => {
